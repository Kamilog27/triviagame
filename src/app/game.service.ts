...
export class GameService {
  ...
  async getTriviaQuestion():Promise<ApiResponse>{
    ...
    const data=await this.httpClient.get("http://localhost:4001/questions/single",{headers:{user:this.userService.userName,access_token:this.userService.token}}).toPromise()
  ...
  sendUserResponse(question:string,answer:string){
    ...
    headers:{
      ...
      access_token:this.userService.token
    } 
    }).toPromise()
  }
}
